...
namespace Components {
  ...
  struct SocketCfg {
    ...
    // Create a TCP / UDP socket to either coonect to or listen for data on or listen for connections
    // on the specified interface and IP:port information
    [[nodisregard]] inline auto createSocket(Logger &logger, const SocketCfg &socket_cfg) -> int {
      ...
      for (addrinfo *rp = result; rp; rp = rp->ai_next) {
        ...
        if (!socket_cfg.is_listening_) { // allow re-using the address in the call to bind()
          ASSERT(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char *>(&one), sizeof(one)) == 0, "setsockpt() SO_REUSEADDR failed. errno:" + std::string(strerror(errno)));        
        }
        ...
        if (socket_cfg.is_listening_) {
          ...
          ASSERT(bind(socket_fd, socket_cfg.is_udp_ ? reinterpret_cast<const struct sockaddr *>(&addr) :
                 rp->ai_addr, sizeof(addr)) == 0, "bind() failed. errno:" + std::string(strerror(errno)));
        }
