...
namespace Components
{
    ...
    {
    public:
        explicit MemPool(std::size_t num_elements) : store_(num_elements, {T(), true}) /* pre-allocation of vector storage */
        {
        ...
        auto deallocate(const T *elem) noexcept
        {
            ...
            ASSERT(!store_[elem_index].is_free_, "Eexpected in-use ObjectBlock at index " + std::to_string(elem_index));
            store_[elem_index].is_free_ = true;

            // Not delete called because it is expected to be overwritten

            // Alternatively Explicitly call the destructor, but why not?
            // 1. Complexity of the Destructor
            // 2. Frequency of the Deallocations (overheads can add up to be computationally expensive)
            // 3. Inlining and Optimization
            // store_[elem_index].object_.~T();

            // Another alternative
            // Only call the destructor if necessary
    ...
    private:
        ...
        // Consider how there are accessed and cache performance
        struct ObjectBlock
        {
            T object_;
            bool is_free_ = true;
